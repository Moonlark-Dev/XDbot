...
for module in submodules:
    if module not in config["entertainment"]["disabled"]:
        try:
            plugin_modules[module] = __import__(f"src.plugins.Core.entertainment.{module}")
...
logger.success(f"Loaded {plugin_modules.keys().__len__()}/{submodules.__len__()} submodules.")
